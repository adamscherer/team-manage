...
export class MemStorage implements IStorage {
  ...
  async getTimeEntries(filter: TimeEntryFilter): Promise<TimeEntry[]> {
    ...
    if (filter.startDate !== undefined) {
      entries = entries.filter(
        (entry) => new Date(entry.date) >= filter.startDate!
      );
    }

    if (filter.endDate !== undefined) {
      entries = entries.filter(
        (entry) => new Date(entry.date) <= filter.endDate!
      );
    }
  ...
  async updateTimeEntry(
    id: number,
    insertTimeEntry: InsertTimeEntry
  ): Promise<TimeEntry | undefined> {
  ...
  async getStats(
    ...
    const projectEntries = new Map<
      number,
      { minutes: number; project: Project | undefined }
    >();

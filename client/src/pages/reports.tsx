  ...
  return (
    <div className="p-6">
      <header className="mb-8">
        ...
        <p className="text-muted-foreground">Analyze your time tracking data and generate reports.</p>
      </header>
            ...
            <Card>
              ...
              <CardContent className="h-80">
                {isLoadingStats ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart layout="vertical" data={prepareProjectChartData()}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis type="number" />
            ...
            <CardContent>
              {isLoadingStats ? (
                ...
                <div className="text-center py-12">
                  <p className="text-muted-foreground">No project data available for the selected date range.</p>
                </div>
              ) : (
                  ...
                  {stats.projectBreakdown.map((project) => (
                    <div key={project.id} className="border-b pb-4 last:border-0">
                      ...
                      <div className="mt-4 flex justify-between text-sm text-muted-foreground">
                        <div>
                          <span className="font-medium text-foreground">Billable amount:</span> {formatCurrency(project.hours * 150)}
                        </div>
                        <div>
                          <span className="font-medium text-foreground">Tasks:</span> {timeEntries?.filter((entry: any) => entry.projectId === project.id).length || 0}
                        </div>
                      </div>
                    </div>
                  ))}
